...
import { Correct_Answers, getRank, New_Try, nextQS } from "../actions/generalActions";
...
const Options = () => {
  ...
  const { correctAnswer, order, QSNumber,correct } = useSelector(mapStateToProps);
  console.log(correctAnswer,order,QSNumber,correct)
  ...
  const submitAnswer = () => {
      ...
      if (correctAnswer === choice) {// if correct answer
        dispatch(Correct_Answers());
        console.log(true)
        document.querySelector(`.${choice}`).innerHTML = "true";
    } 
    if(correctAnswer!== choice ) { // if rong answer
  ...
  return (
    <div className="bottom">
      ...
      <Buttons submitAnswer={submitAnswer}
       next={next}
